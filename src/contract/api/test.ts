...
import { parseTokenMeta, sleep, getDeploy, getAccountInfo, getAccountNamedKeyValue } from "../contract/utils";
//import { cep47, CEP47Events, CEP47EventParser } from "../contract/cep47";
import { cep47 } from "../../lib/cep47";
import { getUserNFTs, getUserNFTsImages, getUserTokens } from "./nftUtils";

import { getNFTsOwned } from "./userInfo";
...
const test = async () => {
  ...
  let accountInfo = await getAccountInfo(NODE_ADDRESS!, "0117db225b72cc7fd1d8ce8bf7c62670919e4e45cd7cdfc2a31868d897e33c6020");
  ...
  // console.log(`... Total supply: ${totalSupply} add 1 = ${parseInt(totalSupply as string) + 1}`);
  ...
  //   KEYS.publicKey,
  //   [`${parseInt(totalSupply as string) + 1}`],
  //   [new Map([['number', 'one']])],
  //   MINT_ONE_PAYMENT_AMOUNT!,
  //   KEYS.publicKey,
  //   [KEYS]
...
// const address = "012e37ffd943f25cf1fa6f0fbb3bdca845cbc468b08a34f3d5752ae03ef5dd07a1"
const myAddress = "0117db225b72cc7fd1d8ce8bf7c62670919e4e45cd7cdfc2a31868d897e33c6020"

const a = async () => {
  const res = await getUserNFTs(myAddress);
  console.log(res);
};
...
// metadata[0].set('Name', "Dragon Egg");
// metadata[0].set('Description', "Dragon Egg");
// metadata[0].set('Box', 'Red');
// metadata[0].set('Skin', 'Orange');
// metadata[0].set('Magic Spot', 'Lemon');
// metadata[0].set('Guardian Royalty', 'Nefertiti');
// metadata[0].set('City Name', 'A k h e t a t e n');
// metadata[0].set('Glow Effect', 'Blue');
// metadata[0].set('Image', 'https://scarlet-adequate-clownfish-387.mypinata.cloud/ipfs/QmSV7yUfie3n9Rb2U5n6Nu6dQGPD7sHdwvGU3NGVCEPgmW/1_Red_Orange_Lemon_Nefertiti_A%20k%20h%20e%20t%20a%20t%20e%20n_Blue_.png');


// mint(metadata);

// run ts-node src/contract/api/test.ts