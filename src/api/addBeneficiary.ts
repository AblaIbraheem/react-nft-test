...
export async function removeBeneficiary(
  ...
  const beneficiaryDeploy = await cep47.removeBeneficiary(
    CLValueBuilder.byteArray(Buffer.from(address_hash, 'hex')),
    PAYMENT_AMOUNTS.MINT_ONE_PAYMENT_AMOUNT,
    deploySender
  );
  console.log('Beneficiary removal deploy:', beneficiaryDeploy);

  const signedBeneficiaryDeploy = await signDeploy(
    beneficiaryDeploy,
    deploySender
  );
  console.log('Signed Beneficiary removal deploy:', signedBeneficiaryDeploy);
